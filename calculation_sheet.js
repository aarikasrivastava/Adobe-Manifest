...
function increasedTransactions(Number_of_annual_transactions) {
  number_increased_transaction = ((parseFloat(Number_of_annual_transactions) + parseFloat(totalDigitalEnrollment)) * 0.50).toFixed();
...
function customerExperienceValue(Number_of_annual_transactions) {
  ...
  customer_value=((parseFloat(a) + parseFloat(b)+parseFloat(c)) *0.85) .toFixed();
  return  customer_value ;
}

function complianceEfficiency(){
  compliance = parseFloat(((employeesComplianceRegulatory*hoursSpentEmployeeComplianceRegulatory * 52* percentReductionComplianceRegulatory*0.01) * compensationComplianceRegulatoryStaff *0.85)).toFixed();
 return compliance; 
...
function legacyEfficiency(Number_of_annual_transactions) {
  ...
  return parseFloat(legalEff).toFixed();
}
function sustainbilitySaving(Number_of_annual_transactions){
  ...
  b1= a1 *0.50 ; 
  sustainbility = ((parseFloat(a1 * printingScanningFaxingSaving )+ parseFloat(b1*mailingScanningFaxingSaving))*0.85).toFixed();
